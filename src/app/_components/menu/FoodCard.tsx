...
function FoodCard({ title, description, price, rating, imageUrl, id }: FoodCardProps) {
  ...
  const { mutate: addToCart, isPending } = api.basket.addToCart.useMutation({
    onSuccess: () => {
      void refreshCart(); // Обновляем счетчик после успешного добавления
    },
    onError: (error) => {
  ...
  const handleAddToCart = () => {
    if (isPending) return; // Предотвращаем повторные нажатия во время загрузки
    addToCart({ dishId: id });
  };
      ...
      <div className="relative"> 
        ...
        <div className="absolute top-2 right-2">
          <button 
            ...
            disabled={isPending}
            className={`bg-orange-500 p-2 rounded-full transition cursor-pointer ${
